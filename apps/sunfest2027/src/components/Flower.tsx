...
/** Orquídea (Cattleya) — Colombia's national flower. */
function orchid(color: string): ReactElement {
  return (
    <>
      <g fill={color}>
...
/** Veranera (bougainvillea) — three papery bracts around tiny white flowers. */
function veranera(color: string): ReactElement {
  return (
    <>
      <g fill={color}>
        {TRIAD.map((angle) => (
...
/** Flor de café — five slim petals around a center. */
function cafe(color: string): ReactElement {
  return (
    <>
      <g fill={color}>
        {PENTAGON.map((angle) => (
...
/** Guayacán — five broad trumpet lobes around a throat. */
function guayacan(color: string): ReactElement {
  return (
    <>
      <g fill={color}>
        {PENTAGON.map((angle) => (
...
/** Girasol (sunflower) — many yellow rays around a dark center. */
function girasol(): ReactElement {
  return (
    <>
      ...
      <circle cx="50" cy="50" r="17" fill="var(--color-ink)" />
      <circle cx="50" cy="50" r="10" fill="var(--color-purple)" />
    </>
  );
}

/** The inner SVG shapes for one Colombian flower variant. */
function shapes(variant: number, color: string): ReactElement {
  switch (variant % FLOWER_VARIANTS) {
    case 1:
      return veranera(color);
    case 2:
      return cafe(color);
    case 3:
      return guayacan(color);
    case 4:
      return girasol();
    default:
      return orchid(color);
 ...
 * guayacán, or girasol — selected by `variant`.
 */
